...
typedef class Symbol { 
    ...
    public:
        ...
        Symbol (symbol_t type,std::string name,scopespace_t space,unsigned scope,unsigned line,unsigned offset){
            this->isActive=true;
            this->type=type;
...
typedef class ScopeLists {
    private:
        unsigned int scope;
        std::list <Symbol> *S_list;
    public:
        unsigned int getScope(){return scope;}
        std::list <Symbol> *getList(){return S_list;}
    ScopeLists (unsigned int scope,Symbol *symbol){
        S_list = new std::list<Symbol>();
        this->scope=scope;
...
typedef class SymbolTable {
    private:
        std::list <Symbol> *HashTable[BUCKETS];
        std::list <ScopeLists> scopelists;
        unsigned int hash(std::string name);
    public:
        void Insert(Symbol *sym);
        Symbol* Lookup(std::string name); 
        Symbol* LookupScope(std::string name,unsigned int scope);
